...
if [ "$BACKGROUND" = true ]; then
  ...
  tmux new-session -d -s "$SESSION_NAME" "uv run python src/lora_training_xlmr.py --config-name=squad-en-15K-s42_test; $STOP_CMD; exec bash"
  ...
  uv run python src/lora_training_xlmr.py \
    --config-name=squad-en-15K-s42_test
  stop_vastai_instance
fi
